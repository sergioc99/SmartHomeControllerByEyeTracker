using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class CameraRotation5 : MonoBehaviour
{
    public Transform[] views;
    public Transform currentView;
    public Camera camera;
    public GameObject[] newItemsView1;

    public GameObject[] disappearedItemsView1;

    public GameObject[] disappearedItemsView;

    void OnMouseOver()
    {
        if (Input.GetMouseButtonDown(1) || Input.GetMouseButtonDown(0))
        {
            if (camera.transform.position + "" == views[0].position + "")
            {
                currentView.position = views[1].position;
                currentView.eulerAngles = views[1].eulerAngles;
                foreach (GameObject gameObject in newItemsView1)
                {
                    gameObject.SetActive(true);
                }

                foreach (GameObject gameObject in disappearedItemsView1)
                {
                    gameObject.SetActive(false);
                }
            }
            else if (camera.transform.position + "" == views[5].position + "")
            {
                currentView.position = views[1].position;
                currentView.eulerAngles = views[1].eulerAngles;

                foreach (GameObject gameObject in disappearedItemsView)
                {
                    gameObject.SetActive(false);
                }
            }
        }
    }
}